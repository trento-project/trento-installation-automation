...
. **Provision infrastructure & deploy Trento**: provisions Azure VMs, registers SUSE machines, configures package sources from Azure Blob Storage or the Trento rolling repository from https://build.opensuse.org/project/show/devel:sap:trento:factory[OBS], installs Trento with Ansible, configures HTTPS certificates, and verifies health endpoints.
. **Run Cypress tests**: runs Cypress end-to-end tests against every target node.
...
NOTE: When `install_rolling` is disabled, packages available in Azure Blob Storage are configured with the highest repository priority. If a package is not provided in blob storage, it will be installed from the official SUSE release sources instead. This is useful when you want to test only a specific package (e.g., Ansible) while installing the rest from official SUSE release sources, or vice versa. When `install_rolling` is enabled, the Trento rolling repository from https://build.opensuse.org/project/show/devel:sap:trento:factory[OBS] is configured with the highest repository priority and Azure Blob Storage is skipped.
...
`TRENTO_TEST_CA_CRT` is public and can be imported locally by developers who need to access the generated Trento environments from a browser. `TRENTO_TEST_CA_KEY` is sensitive and should only be stored in GitHub Actions secrets and the team's password vault.
...
The workflow always recreates the Terraform-managed infrastructure before installing Trento. After installation, Cypress end-to-end tests run against each target node. If Cypress fails, screenshots are uploaded as workflow artifacts.
