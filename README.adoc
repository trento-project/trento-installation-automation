...
NOTE: SSH keys are generated automatically at the start of each deployment. No manual SSH key setup is required.

NOTE: The Azure region is hardcoded to `westeurope`. To use a different region, set `AZURE_VMS_LOCATION` in your `.env` file.
...
. **SSH Key Setup** - Sets up fresh ephemeral SSH keys for the deployment
. **Terraform Provisioning** - Creates Azure VMs using the generated SSH keys
...
NOTE: The `.ssh-keys/` directory contains automatically generated SSH keys used for VM access. These keys are specific to each deployment and can be safely removed after cleanup.
