...
Two behaviours are worth knowing before launching a run:
...
* All runs share the `trento-installation-tests` concurrency group and are never cancelled, so a run started while another one is in progress waits in the queue instead of starting immediately.
...
The storage account and container to upload to are the ones configured in the `AZURE_BLOB_STORAGE` and `AZURE_BLOB_STORAGE_CONTAINER` repository variables.

Before launching the workflow, connect to the VPN and, for each SLES version you plan to deploy to:

. Delete the RPMs left in `<container>/<sles_major_version>/<sp_version>/` by previous runs.
. Upload only the RPMs under validation.

IMPORTANT: Deleting the leftovers is not optional. The blob-backed repository is configured with priority 1, which zypper prefers over the official repositories even when those offer a newer version. An RPM left over from a previous run keeps being installed until it is removed from the container.

For example, to validate a `trento-web` release candidate on SLES 15 SP6, leave `15/6/` containing only `trento-web-*.rpm`. `trento-wanda`, `trento-checks`, `trento-agent` and `ansible-trento` are then installed from the official SUSE release sources.
...
`ansible-trento` is not officially released for SLES 15 SP4, so release candidate artifacts never include an SP4 build of it. Trento can still be installed on an SP4 target, because the SP4 control node does not use the distribution Ansible: it creates a Python virtual environment and installs `ansible` 9 with pip, pointing it at the collection provided by the RPM. The collection therefore works regardless of which service pack it was built for.

When preparing a run that includes SP4, upload the SP5 `ansible-trento` release candidate to `15/4/` alongside the rest of the SP4 packages.
...
Because manually dispatched runs leave the infrastructure in place, each target node stays reachable for manual validation at `+https://target{slesVersion}sp{spVersion}.<azure_region>.cloudapp.azure.com/+`, where `<azure_region>` is the location of `AZURE_RESOURCE_GROUP`. For example, https://target15sp7.westeurope.cloudapp.azure.com/ for SLES 15 SP7. Control nodes follow the same pattern with the `control` prefix.
...
The **Continuous Integration** workflow runs on pushes to `main`, pull request open/update events, and manual dispatches. It calls the reusable installation base workflow with all supported SLES versions enabled, installs packages from the Trento rolling repository, runs only the `+**/about.cy.js+` Cypress spec against every target node, and destroys the Terraform-managed infrastructure after completion.
