...
The **Continuous Integration** workflow runs on pushes to `main`, pull request open/update events, and manual dispatches. It calls the reusable installation base workflow with only SLES 15 SP7 enabled, installs packages from the Trento rolling repository, runs only one Cypress spec, and destroys the Terraform-managed infrastructure after completion.
